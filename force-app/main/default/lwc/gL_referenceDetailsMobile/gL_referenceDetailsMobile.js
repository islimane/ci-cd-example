...
export default class ReferenceDetails extends LightningElement {
	...
	connectedCallback() {
		...
		if (this.referenceDetail.priceWrapper != undefined && this.referenceDetail.priceWrapper.length !== 1) {
			this.hastiers = true;
			this.defaultPriceWithTiers = this.referenceDetail.priceWrapper[0].unitPrice + ' ' + this.referenceDetail.priceByAmount;
		}
	...
	handleAddToCartNotify(refId, addCartNoNotify) {
		...
		.catch((e) => {
			...
			if(e.body != undefined){
				...
				getErrorRecord(errorLabel, this.labels.error5 + ' ' + e.body.message + '. ' + this.labels.adminerror,'GL_referenceListCtrl' );
			} else {
				this.showMessage(errorLabel, this.labels.error5 + '. ' + this.labels.adminerror, 'error');
				getErrorRecord(errorLabel, this.labels.error5 + '. ' + this.labels.adminerror,'GL_referenceListCtrl');
			}
