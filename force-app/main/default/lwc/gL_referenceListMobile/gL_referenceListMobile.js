...
import getMaterialName from '@salesforce/apex/GL_referenceListCtrl.getMaterialName';
...
export default class ReferenceList extends LightningElement {
	...
	connectedCallback() {
		if (basePathName === '/chavesbao/s') {
			this.style = 'mobile-header-chavesbao';
			this.filtersBorderClass = 'filters-border-chavesbao';
			this.ischaves = true;
		} else {
			this.style = 'mobile-header-indexfix';
    ...
    getName() {
        ...
        .then((result) => {
			this.materialName = result;
		})
        .catch((e) => {
			getErrorRecord(errorLabel, 'ERROR GET MATERIAL NAME -> ' + JSON.stringify(e), 'GL_referenceListCtrl');
        });
	}
    ...
    addAllToCart(event) {
		...
		if (this.refMap.length > 0) {
			...
			this.showMessageMethod(errorLabel, this.labels.addToCartWarning, 'info');
		}
	}
