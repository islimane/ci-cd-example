...
export default class CartLineItemMobile extends LightningElement {
    ...
    showCompProductsText = 'Mostrar productos complementarios';//borrar

	defaultPriceWithTiers = '';
	...
	connectedCallback() {
		...
		if(this.cartLine.priceWrapper.length !== 1){
			this.hastiers = true;
			this.defaultPriceWithTiers = this.cartLine.priceWrapper[0].unitPrice + ' ' + this.cartLine.priceByAmount;
		}
	...
	getlabels() {
		...
		this.labels = {
			...
			info : labelList[11],
			case : labelList[12],
			material : labelList[13],
			description : labelList[14],
			customer : labelList[15],
	...
	handleShowMoreInfo() {
		if (this.showMorePricing && !this.showMoreInfo) {
			this.showMorePricing = false;
	...
	handleShowMorePricing() {
		if (this.showMoreInfo && !this.showMorePricing) {
			this.showMoreInfo = false;
			this.showMorePricing = true;
		} else {
			this.showMorePricing = !this.showMorePricing;
		}
	}
}